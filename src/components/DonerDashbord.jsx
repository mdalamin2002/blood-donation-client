...
export default function DashboardHome() {
  ...
  useEffect(() => {
    if (user?.email) {
      axios
        ...
        .then((res) => {
          setRequests(res.data.reverse().slice(0, 3));
        });
    }
  }, [user]);

  const handleDelete = (id) => {
    axios.delete(`https://mission-scic11-server-template.vercel.app/api/donation-requests/${id}`).then(() => {
  ...
  const handleStatusUpdate = (id, status) => {
    axios.patch(`https://mission-scic11-server-template.vercel.app/api/donation-requests/${id}`, {
                ...
                {requests.map((req) => (
                  <tr key={req._id}>
                    ...
                    <td className="flex gap-1 flex-wrap">
                      ...
                      {["done", "canceled"].includes(req.donationStatus) ? null : (
                        <>
                          <button
                            className="btn btn-xs btn-info"
                            onClick={() => navigate(`/editDonationRequest/${req._id}`)}
                          >
                            Edit
                          </button>
                          <button
